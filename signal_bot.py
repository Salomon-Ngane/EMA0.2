...
async def run_scan_job():
    ...
    for symbol, config in list(STATE.items()):
        ...
        try:
            ...
            if df is None or df.empty:
                LAST_SCAN_LOGS["details"].append(f"❌ <b>{symbol}</b> : données indisponibles.")
                continue
    ...
    for sig in new_signals:
        ...
        message = (
            f"🚨 <b>Nouveau signal détecté</b> {emoji_dir}\n\n"
            f"<b>Actif :</b> {sig['symbol']}\n"
            f"<b>Direction :</b> {sig['direction']}\n"
            f"<b>Type :</b> {sig['signal_type']}\n"
            f"<b>Prix d'entrée :</b> {sig['price']}\n"
            f"🛑 <b>Stop-Loss :</b> {sig['sl']:.4f}\n"
            f"🎯 <b>Take-Profit :</b> {sig['tp']:.4f}\n"
            f"⚖️ <b>Risk/Reward :</b> {sig['rr']}\n"
    ...
    if new_signals:
        recap = (
            f"✅ <b>Analyse terminée</b> — {heure_affichage}\n\n"
    ...
    try:
        await telegram_app.bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=recap, parse_mode="HTML")
    except Exception as e:
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/add_asset — ajouter un actif (ex: <code>BTC/USDT</code> ou <code>R_100</code>)\n"
...
async def list_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not STATE:
        await update.message.reply_text("📭 Aucun actif suivi pour le moment.", parse_mode="HTML")
        return
    msg = "📋 <b>Actifs actuellement suivis :</b>\n\n"
    for symbol, cfg in STATE.items():
        status = "✅" if cfg.get("enabled", True) else "⏸️"
        type_str = "Deriv" if is_deriv_symbol(symbol) else "Crypto"
        msg += f"{status} <b>{symbol}</b> ({type_str}) — timeframe {cfg.get('timeframe', '4h')}\n"
    await update.message.reply_text(msg, parse_mode="HTML")

async def add_asset_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args
    if len(args) < 1:
        await update.message.reply_text("ℹ️ Usage : <code>/add_asset SYMBOLE [timeframe]</code>\nExemple Crypto : <code>/add_asset SOL/USDT 4h</code>\nExemple Deriv : <code>/add_asset R_100 4h</code>", parse_mode="HTML")
        return
    symbol = args[0].upper()
    ...
    if save_all_state({"assets": STATE, "signal_state": SIGNAL_STATE}):
        await update.message.reply_text(f"✅ <b>{symbol}</b> ajouté à la liste (timeframe {tf}).", parse_mode="HTML")
    else:
        await update.message.reply_text(f"⚠️ <b>{symbol}</b> ajouté localement, mais échec sauvegarde Supabase.", parse_mode="HTML")

async def remove_asset_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args
    if len(args) < 1:
        await update.message.reply_text("ℹ️ Usage : <code>/remove_asset SYMBOLE</code>\nExemple : <code>/remove_asset R_100</code>", parse_mode="HTML")
        return
    symbol = args[0].upper()
    if symbol in STATE:
        del STATE[symbol]
        if save_all_state({"assets": STATE, "signal_state": SIGNAL_STATE}):
            await update.message.reply_text(f"🗑️ <b>{symbol}</b> retiré de la liste de suivi.", parse_mode="HTML")
        else:
            await update.message.reply_text(f"⚠️ <b>{symbol}</b> retiré localement, mais échec sauvegarde Supabase.", parse_mode="HTML")
    else:
        await update.message.reply_text(f"❓ <b>{symbol}</b> ne fait pas partie de la liste actuelle.", parse_mode="HTML")

async def scan_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔍 Analyse manuelle en cours, merci de patienter...", parse_mode="HTML")
    await run_scan_job()

async def gainers_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🚀 Récupération du Top 5 Gainers Crypto (24h)...", parse_mode="HTML")
    movers = await fetch_top_movers(limit=5, fetch_gainers=True)
    if not movers:
        await update.message.reply_text("⚠️ Impossible de récupérer les données pour le moment.", parse_mode="HTML")
        return
    msg = "🔥 <b>Top 5 hausses (24h) :</b>\n\n"
    for item in movers:
        msg += f"🟢 <b>{item['symbol']}</b> : +{item['change']:.2f}% — {item['price']}\n"
    await update.message.reply_text(msg, parse_mode="HTML")

async def losers_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("📉 Récupération du Top 5 Losers Crypto (24h)...", parse_mode="HTML")
    movers = await fetch_top_movers(limit=5, fetch_gainers=False)
    if not movers:
        await update.message.reply_text("⚠️ Impossible de récupérer les données pour le moment.", parse_mode="HTML")
        return
    msg = "🔻 <b>Top 5 baisses (24h) :</b>\n\n"
    for item in movers:
        msg += f"🔴 <b>{item['symbol']}</b> : {item['change']:.2f}% — {item['price']}\n"
    await update.message.reply_text(msg, parse_mode="HTML")
...
async def logs_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not LAST_SCAN_LOGS["timestamp"]:
        await update.message.reply_text("📭 Aucune analyse n'a encore été effectuée.", parse_mode="HTML")
        return
    msg = f"📊 <b>Détail de la dernière analyse</b> — {LAST_SCAN_LOGS['timestamp']}\n\n"
    msg += "🚨 <b>Signaux détectés :</b>\n"
    msg += ("\n".join(f"• {s}" for s in LAST_SCAN_LOGS["signals"]) if LAST_SCAN_LOGS["signals"] else "Aucun.")
    msg += "\n\n🔎 <b>Détail par actif :</b>\n"
